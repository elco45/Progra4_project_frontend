...
.controller('AdminController', ['$scope','$state','ProductoService','UserService','IngresoService', function ($scope,$state, ProductoService, UserService,IngresoService) {
  ...
  $scope.addProductos = function(){
    if (!$scope.producto.id||!$scope.producto.descripcion||!$scope.producto.fecha_ingreso||!$scope.producto.fecha_venc||!$scope.producto.precio||!$scope.producto.cantidad) {
      ...
      if (dias<=0) {
        ...
        ProductoService.PostProductos($scope.producto).then(function(response){
          $scope.producto = {
            id: "",
            descripcion: "",
            fecha_ingreso: "",
  ...
  /*Graficas*/
  /*¨grafica para productos disponibles pie*/
  ...
  /*¨grafica para productos disponibles bar*/
  ...
  $scope.getIngresos = function(){
    if (!$scope.fecha_ingreso.inicial||!$scope.fecha_ingreso.final) {
      ...
      IngresoService.GetIngresos().then(function(response){
